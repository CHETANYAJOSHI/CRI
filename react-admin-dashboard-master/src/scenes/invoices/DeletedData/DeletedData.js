...
const DeletedData = () => {
  ...
  const fetchLiveDataFile = async (accountId) => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:5000/api/account/${accountId}/self-parent-data-file`);
  ...
  const handleSelectChange = (e) => {
    ...
    setSelectedAccount(accountId);
    if (accountId) {
      navigate(`/enrollment/deleted/self?accountId=${accountId}`);
      fetchLiveDataFile(accountId);
    }
  };
  ...
  const handleEditSave = async () => {
    try {
      await axios.put(`http://localhost:5000/api/account/${selectedAccount}/update-self-parent-row`, {
        rowId: editRow.id,
  ...
  const handleDownloadClick = async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:5000/api/account/${selectedAccount}/download-self-parent-file`, {
        responseType: 'blob',
      });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'Self.xlsx');
  ...
  const handleUploadClick = async () => {
    ...
    try {
      await axios.post(`http://localhost:5000/api/account/${selectedAccount}/upload-self-parent-file`, formData, {
        headers: {
  ...
  const handleAddUserSave = async () => {
    try {
      await axios.post(`http://localhost:5000/api/account/${selectedAccount}/add-self-parent-row`, {
...
<Header title={` ${selectedAccountName}`}/>
    ...
    >
      ...
      >
        ...
        <DialogActions>
          <Button onClick={() => setEditRow(null)} color="primary">
            Cancel
          </Button>
