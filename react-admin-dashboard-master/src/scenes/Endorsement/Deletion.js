...
const Deletion = () => {
  ...
  const fetchLiveDataFile = async (accountId) => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:5000/api/account/${accountId}/Deletion`);
  ...
  const handleEditSave = async () => {
    try {
      await axios.put(`http://localhost:5000/api/account/${selectedAccount}/update-Deletion`, {
        rowId: editRow.id,
  ...
  const handleAddUserSave = async () => {
    try {
      await axios.post(`http://localhost:5000/api/account/${selectedAccount}/add-Deletion`, {
