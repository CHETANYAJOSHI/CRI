...
const ClaimAnalysis = () => {
 ...
 // Replace with the actual account ID
  ...
  const handleSelectChange = (e) => {
    ...
    setSelectedAccount(accountId);
    if (accountId) {
      navigate(`/claim/Selfclaimanalysis?accountId=${accountId}`);
      setSelectedAccount(accountId);
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:5000/api/claim-self-analysis/upload/${selectedAccount}`, formData, {
        headers: {
  ...
  const downloadPdf = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/claim-self-analysis/download/${selectedAccount}`, {
        responseType: 'blob',
      });
  ...
  const loadPdfViewer = () => {
    document.getElementById('pdfViewer').src = `http://localhost:5000/api/claim-self-analysis/${selectedAccount}`;
};
