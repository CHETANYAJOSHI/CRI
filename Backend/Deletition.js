...
const getDeletionDataFile = async (req, res) => {
  try {
    ...
    const { accountName, deletionDataFile } = account;
    if (!accountName || !deletionDataFile) {
      return res.status(400).json({ error: 'Invalid account data' });
    }

    const claimsFilePath = path.join(__dirname, 'NewAccounts', deletionDataFile);
...
const downloadDeletionDataFile = async (req, res) => {
  const accountId = req.params.accountId;
  const account = await Accounts.findById(accountId);
  const { deletionDataFile } = account;
  if (!deletionDataFile) {
    return res.status(400).json({ error: 'Invalid account data' });
  }

  if (account) {
    const filePath = path.join(__dirname, './NewAccounts', deletionDataFile);
...
const Deletionupload = multer({ storage });

// Function to upload claims data file
const uploadDeletionDataFile = async (req, res) => {
  if (!req.file) {
  ...
  try {
    ...
    const oldClaimsFilePath = path.join(__dirname, 'NewAccounts', account.accountName, account.deletionDataFile);
...
const AddSelfDeletion = async (req, res) => {
  const { accountId } = req.params;
